...
- **Technical Reference**: [ECMA-262 - this Resolution](https://tc39.es/ecma262/#sec-resolvethisbinding)

## 🎓 Senior Terminology
- **Default Binding**: Aturan di mana `this` merujuk ke objek global jika fungsi dipanggil tanpa konteks (kecuali dalam `strict mode`).
- **Implicit vs Explicit Binding**: Perbedaan antara `this` yang ditentukan otomatis oleh pemanggilan metode vs `this` yang dipaksa menggunakan `call`, `apply`, atau `bind`.
- **New Binding**: Aturan di mana `this` secara otomatis merujuk ke instance objek baru saat fungsi dipanggil sebagai constructor.
