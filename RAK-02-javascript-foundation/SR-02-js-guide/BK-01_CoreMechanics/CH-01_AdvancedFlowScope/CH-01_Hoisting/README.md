...
## 🎓 Senior Terminology
- **Temporal Dead Zone (TDZ)**: Area di mana variabel `let` dan `const` sudah dideklarasikan tapi belum bisa diakses hingga inisialisasi dilakukan.
- **Creation vs Execution Phase**: Dua tahap utama dalam Execution Context; tahap pembuatan (setup memori) dan tahap eksekusi (menjalankan kode).
- **Function Declaration vs Expression**: Perbedaan cara Hoisting bekerja antara fungsi yang dideklarasikan utuh dengan fungsi yang disimpan dalam variabel.
