...
## 🎓 Senior Terminology
- **Lexical Scope**: Penentuan akses variabel berdasarkan lokasi fisik kode tersebut ditulis dalam struktur program (Static Scope).
- **Scope Chain**: Mekanisme pencarian variabel ke arah luar (Parent Scope) hingga mencapai scope global jika tidak ditemukan di scope lokal.
- **Variable Shadowing**: Kondisi di mana variabel di dalam scope lokal memiliki nama yang sama dengan variabel di scope luar, sehingga menutupi akses ke variabel luar tersebut.
